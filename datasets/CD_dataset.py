...
def get_label_path(root_dir, split, img_name):
    ...
    return os.path.join(
        root_dir, split, "label", img_name.replace(".jpg", label_suffix)
    )
...
class ImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        A_path = get_img_path(
            self.root_dir, self.split, self.img_name_list[index % self.A_size]
        )
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        [img, img_B], [label] = self.augm.transform(
            [img, img_B], [label], to_tensor=self.to_tensor, patch=self.patch
        )
